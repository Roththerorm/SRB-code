...
# Use os.path.join to construct the file path
    for l in range(1,2):
        ...
        if not os.path.exists(filepath):
            ...
            # plot_animation(f_data, 100, 250, 0, 3000, 700, 'y', 10, False)

            fig, ax = plt.subplots()
            for i in range(497,498):
                spectrum = f_data.iloc[:, i].values
                peaks, properties = find_peaks(spectrum, height = 700, prominence=10)
                ...
                if peak_values is not None:
                    ...
                    ax.plot(f_data.iloc[100:200, 0], f_data.iloc[100:200, i], lw=1.5, color='firebrick')
                    ax.tick_params(axis='both', which='major', labelsize=12)
                    ax.plot(f_data.iloc[valid_peaks,0], f_data.iloc[valid_peaks,i], 'v', color='black', label='peak')
...
#             # graphs.parisi_histogram(*args_2, title='n', save='n', fig_path=figure_path)
